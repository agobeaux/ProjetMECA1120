...
femMesh *theGlobalMesh;

int compareX(const void *nodeOne, const void *nodeTwo) 
{
    int *iOne = (int *)nodeOne;
    int *iTwo = (int *)nodeTwo;
    double diff = theGlobalMesh->X[*iOne] - theGlobalMesh->X[*iTwo];
    if (diff < 0)    return  1;
    if (diff > 0)    return -1;
                     return  0;  
}

int compareY(const void *nodeOne, const void *nodeTwo) 
{
    int *iOne = (int *)nodeOne;
    int *iTwo = (int *)nodeTwo;
    double diff = theGlobalMesh->Y[*iOne] - theGlobalMesh->Y[*iTwo];
    if (diff < 0)    return  1;
    if (diff > 0)    return -1;
                     return  0;  
}
...
void femDiffusionRenumber(femDiffusionProblem *theProblem, femRenumType renumType)
{
    ...
    switch (renumType) {
        ...
        case FEM_XNUM : 
            ...
            for (i = 0; i < theProblem->mesh->nNode; i++) 
                inverse[i] = i; 
            theGlobalMesh = theProblem->mesh;
            qsort(inverse, theProblem->mesh->nNode, sizeof(int), compareX);
            for (i = 0; i < theProblem->mesh->nNode; i++)
        ...
        case FEM_YNUM : 
            ...
            for (i = 0; i < theProblem->mesh->nNode; i++) 
                inverse[i] = i; 
            theGlobalMesh = theProblem->mesh;
            qsort(inverse, theProblem->mesh->nNode, sizeof(int), compareY);
            for (i = 0; i < theProblem->mesh->nNode; i++)
                theProblem->number[inverse[i]] = i;
            free(inverse);  
            break;
...
// il suffit d'annuler les deux incréments pour le residu et la direction :-)

// Observer qu'il est impossible de resoudre le probleme avec des conditions essentielles non-homogènes
// tel que c'etait implemente dans le devoir :-)
// Il faudrait modifier tres legerement la fonction femDiffusionCompute pour que cela soit possible :-)
// Le premier étudiant, tuteur ou assistant qui me fournit la version corrigée de l'enonce pour que le
// methode des gradients conjuges fonctionne avec des conditions aux limites non-homogenes reçoit respectivement
// un point bonus, une heure d'encadrement bonus ou un repas à l'OnlyWood (ou l'équivalent à Lanzarote )
//
